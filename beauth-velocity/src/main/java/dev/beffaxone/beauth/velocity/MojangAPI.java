...
public class MojangAPI {
    ...
    public MojangAPI(BeAuthVelocity plugin) {
        this.plugin = plugin;
                ...
                .thenApply(response -> {
                    ...
                    if (response.statusCode() == 200) {
                        try {
                            ...
                            plugin.getLogger().error("Mojang JSON error: " + username, e);
                            result = new PremiumResult(false, null, true);
                        }
                    } else if (response.statusCode() == 204 || response.statusCode() == 404) {
                        result = new PremiumResult(false, null, false);
                    } else {
                        plugin.getLogger().warn("Mojang API returned status code " + response.statusCode() + " for " + username);
                        result = new PremiumResult(false, null, true);
                    }
                    ...
                    plugin.getLogger().error("Mojang API request failed for " + username + ": " + ex.getMessage());
