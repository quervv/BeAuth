...
public class MySQLDatabase implements DatabaseManager {
    ...
    private void migrateDatabase(Connection conn) {
        try {
            ...
            try (var rs = meta.getColumns(null, null, "beauth_players", "premium_uuid")) {
                if (rs.next()) {
                    hasPremiumUuid = true;
                }
            }
            if (!hasPremiumUuid) {
                try (PreparedStatement ps = conn.prepareStatement("ALTER TABLE beauth_players ADD COLUMN premium_uuid VARCHAR(36) AFTER uuid;")) {
                    ps.executeUpdate();
                    plugin.getLogger().info("Database migration successful: added premium_uuid column to MySQL.");
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "MySQL migration error", e);
        }
    }
    ...
    public CompletableFuture<Void> savePlayerData(PlayerData data) {
        return CompletableFuture.runAsync(() -> {
            String query = "INSERT INTO beauth_players (uuid, premium_uuid, username, password_hash, premium, last_ip, last_login) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?) " +
