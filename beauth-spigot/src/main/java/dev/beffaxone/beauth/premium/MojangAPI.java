...
public class MojangAPI {
    ...
    private static final long CACHE_DURATION = 30 * 60 * 1000L;

    public MojangAPI(BeAuth plugin) {
        this.plugin = plugin;
        this.cache = new ConcurrentHashMap<>();
        this.httpClient = HttpClient.newBuilder()
                .connectTimeout(Duration.ofMillis(plugin.getConfigManager().getPremiumMojangTimeout()))
                .build();
    }
    ...
    public CompletableFuture<PremiumResult> checkPremiumStatus(String username) {
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .timeout(Duration.ofMillis(plugin.getConfigManager().getPremiumMojangTimeout()))
                .GET()
                .build();
        ...
        return httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(response -> {
                    ...
                    if (!result.isError()) {
                        cache.put(cacheKey, new CacheEntry<>(result, CACHE_DURATION));
                    }
                    return result;
                }).exceptionally(ex -> {
