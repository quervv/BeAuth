...
public class AuthManager {
    ...
    public void handleJoin(Player player) {
        ...
        plugin.getDatabaseManager().getPlayerData(player.getUniqueId()).thenAccept(data -> {
            if (data == null) {
                plugin.getDatabaseManager().getPlayerDataByName(player.getName()).thenAccept(dataByName -> {
                    Bukkit.getScheduler().runTask(plugin, () -> processLoginState(player, ap, dataByName, ip));
                });
            } else {
                Bukkit.getScheduler().runTask(plugin, () -> processLoginState(player, ap, data, ip));
            }
        }).exceptionally(ex -> {
    ...
    private void processLoginState(Player player, AuthPlayer ap, PlayerData data, String ip) {
        if (data != null) {
            ...
            if (data.isPremium() && plugin.getConfigManager().isPremiumAutoLoginEnabled()) {
